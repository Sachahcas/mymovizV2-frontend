...
function Home() {
  ...
  useEffect(() => {
    fetch('https://mymoviz-v2-backend.vercel.app/movies')
  ...
  const movies = moviesData.map((data, i) => {
    ...
    if (data.overview.length > 250){
      truncOverview = data.overview.slice(0,251)+'...'
    } 
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={truncOverview} poster={data.poster_path} voteAverage={data.voteAverage} voteCount={data.voteCount} />;
  });
